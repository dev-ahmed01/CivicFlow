...
- Web: `apps/web` — Next.js 14 App Router (Project Head + Admin roles only — web-only, per spec Part II §2.3/§4.2)
- Mobile: `apps/mobile` — React Native + Expo (Citizen + Engineer roles — native, per spec Part II §2.1/§2.4)
...
- Auth: Phone OTP for Citizens, email/password+JWT for Project Head/Engineer/Admin (Part III §17.1). Session = short-lived JWT access token + refresh token.
...
4. **Nothing is hardcoded that the spec marks admin-configurable**: issue categories, agency-routing matrix, ward boundaries, verification radius, validation caps, thresholds (§7.1, §7.2, §20). These live in DB config tables editable via `/admin/*` endpoints, not enums baked into application logic.
