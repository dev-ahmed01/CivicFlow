...
export default function ReportsPage() {
  ...
  return <>
    <PageHeader eyebrow="Measured performance" title="Reports" description="Agency delivery outcomes calculated from operational records." />
    ...
    {!report && !loading ? <EmptyState title="Choose a reporting period" description="Run the report to view measured agency outcomes." /> : null}
    {report ? <div className="reports-layout"><section className="report-summary" aria-label="Agency report summary"><div><span>Tickets created</span><strong>{report.totals.ticketsCreated}</strong></div><div><span>Tickets resolved</span><strong>{report.totals.ticketsResolved}</strong></div><div><span>Resolution rate</span><strong>{report.totals.resolutionRatePercent}%</strong></div><div><span>Advisory road conflicts</span><strong>{report.totals.roadConflicts}</strong></div></section><p className="simulation-disclosure"><strong>{report.simulatedRestorationCostSaved.label}:</strong> estimated restoration cost saved ₹{report.simulatedRestorationCostSaved.amountInr.toLocaleString("en-IN")}. {report.simulatedRestorationCostSaved.formula}</p><ReportTable title="Resolution by category and agency" rows={report.resolutionTimeByCategoryAgency} /><ReportTable title="Dependency response" rows={report.dependencyResponseByAgency} /><ReportTable title="Citizen closure feedback" rows={report.citizenNotResolvedByAgency} /><ReportTable title="Sequencing outcomes" rows={report.sequencingOutcomesByAgency} /></div> : null}
  </>;
}
