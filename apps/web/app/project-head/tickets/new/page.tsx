...
export default function AgencyTicketPage() {
  ...
  if (createdId) return <section className="portal-panel completion-panel"><span className="success-mark">✓</span><h1>{projectId ? "Planned work created" : "Agency ticket created"}</h1><p>{projectId ? `The work and road intervention were saved. ${roadConflicts.length} advisory road warning${roadConflicts.length === 1 ? "" : "s"} detected; none blocked creation.` : "This ticket skipped citizen validation and entered the Project Head intake queue directly."}</p><code>{projectId ?? createdId}</code><div>{projectId ? <Link className="primary-link" href={`/project-head/projects/${projectId}`}>Open work</Link> : <Link className="primary-link" href={`/project-head/tickets/${createdId}`}>Open ticket</Link>}<button className="secondary" type="button" onClick={() => { setCreatedId(undefined); setProjectId(undefined); setRoadConflicts([]); setDescription(""); setEvidence(undefined); }}>Create another</button></div></section>;
  ...
  return <><header className="portal-heading"><div><p className="eyebrow">W-P9 · Agency originated</p><h1>Create a civic work ticket</h1><p>Record planned or field-identified work without running a citizen validation cycle.</p></div></header><form className="portal-form" onSubmit={(event) => void submit(event)}>
    ...
    <section className="form-submit"><span className="form-index">{roadEnabled ? "05" : "04"}</span><div><h2>{roadEnabled ? "Create planned work" : "Route to intake"}</h2><p>{roadEnabled ? "The work and road intervention are created together. Advisory checks never prevent saving." : "No reporter is attached and no citizen-validation states are created."}</p>{error ? <p className="error" role="alert">{error}</p> : null}<PrimaryButton disabled={busy || !evidence || (roadEnabled && (!segmentId || !plannedStart || !plannedEnd))} type="submit">{busy ? "Creating…" : roadEnabled ? "Create planned work" : "Create and route ticket"}</PrimaryButton></div></section>
  </form></>;
}
