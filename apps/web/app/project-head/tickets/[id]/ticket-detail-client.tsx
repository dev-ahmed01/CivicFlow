  ...
  return (
    <>
      ...
      <div className="detail-grid">
        <section className="portal-panel"><p className="eyebrow">Ticket record</p><dl className="detail-list"><div><dt>Ticket ID</dt><dd><code>{ticket.id}</code></dd></div><div><dt>Origin</dt><dd>{ticket.reporterId ? "Citizen validated" : "Agency originated"}</dd></div><div><dt>Ward</dt><dd>{ticket.ward.name}</dd></div><div><dt>Observations</dt><dd>{ticket.observationCount}</dd></div></dl>
        ...
        <aside>
          ...
          {ticket.internalState === "INSPECTION_COMPLETE" && !ticket.project ? <section className="portal-panel action-card"><p className="eyebrow">W-P6 · Ready</p><h2>Create delivery project</h2><p>Review this ticket and assign an Executive Engineer from your agency.</p><Link className="primary-link" href={`/project-head/projects/new?ticketId=${ticket.id}`}>Create project</Link></section> : null}
          {ticket.project ? <section className="portal-panel action-card"><p className="eyebrow">Project created</p><h2>{ticket.project.state.replaceAll("_", " ")}</h2><Link href={`/project-head/projects/${ticket.project.id}`}>Open project →</Link></section> : null}
        </aside>
      </div>
    </>
  );
