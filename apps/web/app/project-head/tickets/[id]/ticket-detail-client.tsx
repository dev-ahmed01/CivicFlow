  ...
  return (
    <>
      ...
      <div className="detail-grid">
        ...
        <aside>
          {ticket.action ? <section className="portal-panel action-card"><p className="eyebrow">Response deadline</p><h2>{ticket.action.type.replaceAll("_", " ")}</h2><p>Responsible: {ticket.action.responsibleUser.email ?? "Agency queue"}</p><strong>{new Date(ticket.action.deadline).toLocaleString("en-IN")}</strong></section> : null}
          {ticket.grievances.map((grievance) => <section className="portal-panel grievance-card-escalated" key={grievance.id}><p className="eyebrow">Grievance · {grievance.status.replaceAll("_", " ")}</p><h2>{grievance.reason.replaceAll("_", " ")}</h2><p>Created {new Date(grievance.createdAt).toLocaleDateString("en-IN")}</p><Link className="primary-link" href={`/project-head/grievances?grievance=${grievance.id}`}>Review Grievance</Link></section>)}
