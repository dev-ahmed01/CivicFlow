...
export default function ProjectHeadProfilePage() {
  ...
  return <><header className="portal-heading"><div><p className="eyebrow">Account</p><h1>Project Head profile</h1><p>Your account is provisioned for your agency's operational scope.</p></div></header><Card><dl className="detail-list"><div><dt>Email</dt><dd>{user?.email ?? "Unavailable"}</dd></div><div><dt>Role</dt><dd>Project Head</dd></div><div><dt>Agency scope</dt><dd>{user?.agencyId ?? "Unavailable"}</dd></div></dl></Card></>;
}
