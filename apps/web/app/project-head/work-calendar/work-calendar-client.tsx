...
export function WorkCalendarClient() {
  ...
  const visibleWorks = useMemo(() => works.filter((work) => period === "ALL" || work.period === period), [period, works]);
  ...
  const openHistory = () => {
    ...
    else if (selected?.ward) { setWardId(selected.ward.id); setRoadSegmentId(""); }
    setHistoryOpen(true);
  };
