...
export function ProjectCreateClient({ ticketId, onCreated }: { ticketId: string; onCreated?: (project: Project) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const intervention = roadEnabled && (!demo || Boolean(segmentId && plannedStart && plannedEnd)) ? { segmentId, purpose, plannedStart: `${plannedStart}T00:00:00.000Z`, plannedEnd: `${plannedEnd}T23:59:59.999Z`, affectedLengthM: Number(affectedLengthM), startOffsetM: Number(startOffsetM), dependencyRefs: interventionDependencyRefs } : undefined;
      const result = await apiFetch<{ project: Project }>("/projects", { method: "POST", body: JSON.stringify({ ticketId, engineerId: engineerId || undefined, dependencies, intervention }) });
  ...
  return <><header className="portal-heading"><div><p className="eyebrow">W-P6 · Create project</p><h1>Review and assign</h1><p>Confirm the inspected work and choose an Executive Engineer from your agency roster.</p></div></header><form className="project-create-flow" onSubmit={(event) => void submit(event)}>
    <div className="review-grid"><section className="portal-panel"><p className="eyebrow">Inspected ticket</p><h2>{ticket?.title ?? "Loading ticket…"}</h2>{ticket ? <dl className="detail-list"><div><dt>Ticket</dt><dd><code>{ticket.id}</code></dd></div><div><dt>Category</dt><dd>{ticket.category.name}</dd></div><div><dt>Ward</dt><dd>{ticket.ward.name}</dd></div><div><dt>Status</dt><dd>{ticket.internalState.replaceAll("_", " ")}</dd></div></dl> : null}</section><section className="portal-panel assignment-card"><p className="eyebrow">Agency roster</p><h2>Assign Executive Engineer</h2><label>Engineer<select required={!demo} value={engineerId} onChange={(event) => setEngineerId(event.target.value)}><option value="">{demo ? "Automatic assignment within your agency" : "Choose engineer"}</option>{engineers.map((engineer) => <option key={engineer.id} value={engineer.id}>{engineer.email}</option>)}</select></label><p className="portal-muted">Only engineers belonging to your agency are accepted by the server.</p></section></div>
    {roadEnabled ? <section className="portal-panel road-assessment"><p className="eyebrow">Phase 8 · Exact segment</p><h2>Road intervention details</h2>{demo ? <p>Leave the road fields blank to use the nearest mapped road in this ward, a two-day repair window, and 25 metres or less. No dependencies are added.</p> : null}<p className="portal-muted">The Project timeline mirrors these dates. Existing segment work can be declared as a dependency; nothing is inferred automatically.</p><div className="two-column"><label>Road segment<select required={!demo} value={segmentId} onChange={(event) => { setSegmentId(event.target.value); setInterventionDependencyRefs([]); }}><option value="">Choose segment</option>{segments.map((segment) => <option key={segment.id} value={segment.id}>{segment.roadName} · {segment.ward.name}</option>)}</select></label><label>Purpose<select required={!demo} value={purpose} onChange={(event) => setPurpose(event.target.value as InterventionPurpose)}>{(["pipeline", "cable", "OFC", "resurfacing", "other"] satisfies InterventionPurpose[]).map((item) => <option key={item}>{item}</option>)}</select></label><label>Planned start<input required={!demo} type="date" value={plannedStart} onChange={(event) => setPlannedStart(event.target.value)} /></label><label>Planned end<input required={!demo} min={plannedStart} type="date" value={plannedEnd} onChange={(event) => setPlannedEnd(event.target.value)} /></label><label>Start offset (m)<input required={!demo} min="0" type="number" value={startOffsetM} onChange={(event) => setStartOffsetM(event.target.value)} /></label><label>Affected length (m)<input required={!demo} min="1" type="number" value={affectedLengthM} onChange={(event) => setAffectedLengthM(event.target.value)} /></label></div>{history.length ? <fieldset className="intervention-dependencies"><legend>Declared dependencies on this segment</legend>{history.filter((item) => item.projectId !== ticket?.project?.id).map((item) => <label key={item.id}><input type="checkbox" checked={interventionDependencyRefs.includes(item.id)} onChange={(event) => setInterventionDependencyRefs((current) => event.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))} />{item.requestingAgency.name} · {item.purpose}</label>)}</fieldset> : null}</section> : null}
