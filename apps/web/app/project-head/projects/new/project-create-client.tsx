...
export function ProjectCreateClient({ ticketId, onCreated }: { ticketId: string; onCreated?: (project: Project) => void }) {
  ...
  if (created) return <section className="portal-panel completion-panel"><span className="success-mark">✓</span><p className="eyebrow">Engineer assigned</p><h1>Project created</h1><p>The ticket is now in ENGINEER ASSIGNED. Road warnings remain advisory and dependency requests are waiting in the target agencies’ inboxes.</p><code>{created.id}</code><div><Link className="primary-link" href={`/project-head/projects/${created.id}`}>Review project</Link><Link className="secondary-link" href="/project-head/dependencies/outbox">Dependency outbox</Link></div></section>;
