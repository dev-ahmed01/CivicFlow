...
export function DependencyTable({ direction }: { direction: "sent" | "received" }) {
  ...
  return <><section className="filter-bar dependency-filter"><label>Status<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{statuses.map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select></label></section>{error ? <p className="error" role="alert">{error}</p> : null}<section className="dependency-list">{dependencies.map((dependency) => {
    ...
    return <article className="dependency-row" key={dependency.id}><div className="dependency-main"><div className="dependency-top"><span className={`state-chip ${pending || dependency.state === "ESCALATED" ? "due" : ""}`}>{direction === "sent" ? outboxLabel(dependency.state) : dependency.state.replaceAll("_", " ")}</span>{pending ? <strong className="response-flag">Response required</strong> : null}</div><h2>{dependency.project.ticket?.title ?? "Agency coordination"}</h2><p>{dependency.requirement}</p><dl><div><dt>{direction === "received" ? "Requested by" : "Sent to"}</dt><dd>{otherAgency.name}</dd></div><div><dt>Deadline</dt><dd>{new Date(dependency.deadline).toLocaleString("en-IN")}</dd></div><div><dt>Countdown</dt><dd className={pending ? "countdown" : ""}>{pending ? countdown(dependency.deadline, now) : "Response recorded"}</dd></div>{dependency.assignedEngineer ? <div><dt>Assigned Engineer</dt><dd>{dependency.assignedEngineer.email}</dd></div> : null}</dl>{dependency.contacts.length > 0 ? <div className="escalation-contact"><strong>Escalation contact</strong>{dependency.contacts.map((contact) => <a href={`mailto:${contact.email}`} key={contact.email}>{contact.email}</a>)}</div> : null}</div><div className="dependency-actions">{direction === "received" && pending ? <><label>Response<select aria-label={`Response for ${dependency.id}`} value={choice} onChange={(event) => setChoices({ ...choices, [dependency.id]: event.target.value as InboxChoice })}><option value="">Choose response</option><option value="ASSIGN_ENGINEER">Assign Engineer</option><option value="DECLINE_UNAVAILABLE">Unavailable</option><option value="DECLINE_NOT_CONCERNED">Not Our Scope</option></select></label>{choice === "ASSIGN_ENGINEER" ? <label>Engineer<select value={engineerIds[dependency.id] ?? ""} onChange={(event) => setEngineerIds({ ...engineerIds, [dependency.id]: event.target.value })}><option value="">Choose engineer</option>{engineers.map((engineer) => <option key={engineer.id} value={engineer.id}>{engineer.email}</option>)}</select></label> : null}<button disabled={!choice || busyId === dependency.id || choice === "ASSIGN_ENGINEER" && !engineerIds[dependency.id]} type="button" onClick={() => void respond(dependency.id, choice, engineerIds[dependency.id])}>{busyId === dependency.id ? "Saving…" : "Submit response"}</button></> : null}{direction === "sent" && dependency.state === "DECLINED_UNAVAILABLE" ? <button disabled={busyId === dependency.id} type="button" onClick={() => void respond(dependency.id, "RESEND")}>Re-send request</button> : null}{direction === "sent" && dependency.state === "ESCALATED" ? <button disabled={busyId === dependency.id} type="button" onClick={() => void respond(dependency.id, "MARK_ASSIGNED_OUT_OF_BAND")}>Mark assigned out-of-band</button> : null}{dependency.state === "DECLINED_NOT_CONCERNED" ? <p className="terminal-note">Terminal response · manually choose another agency if needed.</p> : null}</div></article>;
