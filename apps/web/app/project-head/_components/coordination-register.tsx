...
export function CoordinationRegister({ initialView = "NEEDS_RESPONSE" }: { initialView?: CoordinationView }) {
  ...
  return <div className="ph-coordination-page">
    <PageHeader title="Coordination" description="Formal inter-agency requests, responses, conflicts, and recorded decisions." />
    ...
    {!loading && view === "CONFLICTS" ? <section className="ph-conflict-register">{conflictGroups.length ? <ol>{conflictGroups.map((group) => { const conflict = group[0]; const coordination = group.find((item) => item.coordination)?.coordination; return <li key={conflictPairKey(conflict)}><button className="ph-conflict-open" onClick={() => setSelectedConflict(group)} type="button"><span className="ph-conflict-place"><small>Location</small><strong>{conflict.locationDescription}</strong><span>{conflict.sourceWork.agency.name} ↔ {conflict.conflictingWork.agency.name}</span></span><span className="ph-conflict-work"><small>{conflict.sourceWork.agency.name}</small><strong>{conflict.sourceWork.title}</strong><span>{dateRange(conflict.sourceWork)}</span></span><span className="ph-conflict-work"><small>{conflict.conflictingWork.agency.name}</small><strong>{conflict.conflictingWork.title}</strong><span>{dateRange(conflict.conflictingWork)}</span></span><span className="ph-conflict-reason"><small>Overlap reason</small><strong>{label(conflict.temporalRelationship)}</strong><span>{conflict.reason}</span><small>{group.length} deterministic rule{group.length === 1 ? "" : "s"} matched</small></span><span className="ph-conflict-action"><StatusChip label={coordination ? label(coordination.status) : "Coordination needed"} tone={coordination ? undefined : "warning"} /><b>Inspect conflict →</b></span></button></li>; })}</ol> : <EmptyState title="No advisory conflicts" description="Deterministic spatial and schedule checks will appear here with their rule-based reason." />}</section> : null}
