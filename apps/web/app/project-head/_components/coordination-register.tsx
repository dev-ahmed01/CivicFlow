...
function requestLocation(request: CoordinationRequest): string {
  return getShortWorkLocation({ locationLabel: request.project.locationLabel, title: request.project.title, ward: request.project.ward ?? undefined });
}
    ...
    <div className="ph-coordination-workspace">
      <section className="ph-surface ph-coordination-master" aria-label="Coordination records">
        ...
        {view !== "CONFLICTS" ? <div className="ph-coordination-items">{requests.map((request) => {
          ...
          return <button className="ph-coordination-item" aria-pressed={!creating && selectedRequest?.id === request.id} onClick={() => {setSelectedRequestId(request.id);setCreating(false);}} key={request.id} type="button"><span className="ph-avatar" aria-hidden="true">{partner.name.replace(/[^a-zA-Z]/g, "").slice(0,2).toUpperCase()}</span><span className="ph-coordination-item-copy"><strong>{partner.name}</strong><small>{partner.type}</small><b>{request.subject}</b><WorkSummary reference={request.project.referenceNumber} title={request.project.title} location={requestLocation(request)} /><span className="ph-coordination-item-footer"><StatusChip label={label(request.status)} /><small>Respond by {new Date(request.responseDeadline).toLocaleDateString("en-IN")}</small></span><span className="ph-text-action">{action} →</span></span></button>;
