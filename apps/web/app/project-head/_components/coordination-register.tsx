...
export function CoordinationRegister({ initialView = "NEEDS_RESPONSE" }: { initialView?: CoordinationView }) {
  ...
  return <div className="ph-coordination-page">
    <PageHeader title="Coordination" description="Formal inter-agency requests, responses, conflicts, evidence, and decisions linked to civic work." />
    ...
    {!loading && view === "CONFLICTS" ? <section className="ph-conflict-register">{conflictGroups.length ? <ol>{conflictGroups.map((group) => { const conflict = group[0]; const coordination = group.find((item) => item.coordination)?.coordination; return <li key={conflictPairKey(conflict)}><div className="ph-conflict-place"><strong>{conflict.locationDescription}</strong><span>{conflict.sourceWork.agency.name} ↔ {conflict.conflictingWork.agency.name}</span></div><div className="ph-conflict-works"><strong>{conflict.sourceWork.title}</strong><span>{dateRange(conflict.sourceWork)}</span><strong>{conflict.conflictingWork.title}</strong><span>{dateRange(conflict.conflictingWork)}</span></div><div className="ph-conflict-reason"><strong>{conflict.temporalRelationship}</strong><details><summary>Rule details ({group.length})</summary><ul>{group.map((rule) => <li key={`${rule.kind}:${rule.id}`}><strong>{rule.kind === "ROAD" ? `Road conflict · ${label(rule.roadConflictType ?? "spatial")}` : `Schedule and location · ${label(rule.severity)}`}</strong><span>{rule.reason}</span></li>)}</ul></details></div><div className="ph-conflict-action"><StatusChip label={coordination ? label(coordination.status) : "Coordination needed"} tone={coordination ? undefined : "warning"} /><Link className="ph-row-action" href={coordination ? `/project-head/coordination/${coordination.requestId}` : `/project-head/projects/${conflict.projectId}`}>{coordination ? "Open" : "Coordinate"} →</Link></div></li>; })}</ol> : <EmptyState title="No advisory conflicts" description="Deterministic spatial and schedule checks will appear here with their rule-based reason." />}</section> : null}
  </div>;
}
