...
export function InspectionDetailClient({ inspectionId }: { inspectionId: string }) {
  ...
  return <div className="field-module inspection-detail">
    ...
    <section className="inspection-context"><header><p className="eyebrow">{inspection.source === "AREA_SCAN" ? "Source: Area Scan" : "Reported issue context"}</p><h2>What needs inspection</h2></header><dl><div><dt>Category</dt><dd>{inspection.ticket.category.name}</dd></div><div><dt>Ward</dt><dd>{inspection.ticket.ward.name}</dd></div><div><dt>Road</dt><dd>{inspection.ticket.roadSegment?.roadName ?? "Not linked"}</dd></div><div><dt>Deadline</dt><dd>{new Date(inspection.deadline).toLocaleString("en-IN")}</dd></div></dl><div className="inspection-observations">{inspection.ticket.observations.map((item) => <figure key={item.id}><Image alt="Reported site evidence" height={320} src={item.imageUrl} unoptimized width={480} /><figcaption>{item.note ?? item.address ?? "Reported evidence"}</figcaption></figure>)}</div></section>
