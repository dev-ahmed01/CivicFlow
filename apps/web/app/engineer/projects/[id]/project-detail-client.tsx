...
export function EngineerProjectDetailClient({ projectId }: { projectId: string }) {
  ...
  const [reassignmentReason, setReassignmentReason] = useState("AVAILABILITY");
  ...
  const saveTimeline = (event: FormEvent) => {
    ...
    void mutate(() => apiFetch(`/projects/${projectId}/timeline`, { method: "PATCH", body: JSON.stringify({ plannedStart: start ? `${start}T00:00:00.000Z` : undefined, plannedEnd: end ? `${end}T23:59:59.999Z` : undefined, workDescription: description, dependencyFlags: flags.split(",").map((item) => item.trim()).filter(Boolean) }) }));
  };
  ...
  const requestReassignment = (event: FormEvent) => {
    event.preventDefault();
    void mutate(() => apiFetch(`/projects/${projectId}/reassignment-requests`, { method: "POST", body: JSON.stringify({ reason: reassignmentReason, note: reassignmentNote || undefined }) }));
  };
  const reportBlocker = (event: FormEvent) => {
    event.preventDefault();
    void mutate(async () => {
      await apiFetch(`/projects/${projectId}/blockers`, { method: "POST", body: JSON.stringify({ title: blockerTitle, details: blockerDetails, severity: blockerSeverity }) });
      setBlockerTitle(""); setBlockerDetails("");
    });
  };
  ...
  return <>
    ...
    <div className="engineer-detail-grid"><div>
      ...
      <section className="portal-panel engineer-section"><p className="eyebrow">Inspection</p><h2>Structured assessment</h2>{project.ticket?.inspectionReports.length ? project.ticket.inspectionReports.map((report) => <article className="evidence-row" key={report.id}><strong>{report.status.replaceAll("_", " ")}</strong><p>{report.observations ?? report.notes ?? "Assessment awaiting submission"}</p><small>{report.recommendation?.replaceAll("_", " ") ?? "No recommendation yet"}</small>{report.fileUrl ? <a href={report.fileUrl} rel="noreferrer" target="_blank">Open legacy evidence</a> : null}</article>) : <p className="portal-muted">No inspection is linked to this work.</p>}</section>
      ...
      {project.editable && project.state === "PENDING_UPTAKE" ? <><section className="portal-panel action-card"><p className="eyebrow">Work request</p><h2>Accept this project?</h2><p>Review the ticket, inspection, and dependency context before uptake.</p><button className="engineer-primary" disabled={busy} type="button" onClick={() => void mutate(() => apiFetch(`/projects/${projectId}/uptake`, { method: "POST" }))}>Accept Assignment</button></section><details className="portal-panel engineer-disclosure"><summary>Request reassignment</summary><form className="engineer-form" onSubmit={requestReassignment}><p>This asks your Project Head to decide. It does not cancel the civic work.</p><label>Reason<select value={reassignmentReason} onChange={(event) => setReassignmentReason(event.target.value)}><option value="AVAILABILITY">Availability</option><option value="WORKLOAD">Workload</option><option value="SKILL_MISMATCH">Skill mismatch</option><option value="LOCATION">Location</option><option value="OTHER">Other</option></select></label><label>Note (optional)<textarea value={reassignmentNote} onChange={(event) => setReassignmentNote(event.target.value)} /></label><button className="engineer-secondary" disabled={busy} type="submit">Send Request</button></form></details></> : null}
      {canEditTimeline ? <form className="portal-panel engineer-form action-anchor" id="plan" onSubmit={saveTimeline}><p className="eyebrow">Work plan</p><h2>{project.state === "UPTAKEN" ? "Set timeline" : "Update timeline"}</h2><label>Planned start<input type="date" required={!demo} value={start} onChange={(event) => setStart(event.target.value)} /></label><label>Planned end<input type="date" required={!demo} value={end} onChange={(event) => setEnd(event.target.value)} /></label><label>Work description<textarea required={!demo} minLength={demo ? undefined : 10} value={description} onChange={(event) => setDescription(event.target.value)} /></label><label>Dependency flags<input value={flags} onChange={(event) => setFlags(event.target.value)} placeholder="Comma-separated flags" /></label><small>Saving runs advisory conflict checks. It does not start execution or set the actual start date.</small><button className="engineer-primary" disabled={busy} type="submit">Save Timeline</button></form> : null}
      {project.editable && project.state === "READY_TO_START" ? <section className="portal-panel action-card action-anchor engineer-start-work" id="start"><p className="eyebrow">Ready to start</p><h2>Begin field execution</h2><p>The plan and advisory conflict checks are recorded. Starting work sets the actual start time and updates the linked citizen workflow.</p><button className="engineer-primary" disabled={busy} type="button" onClick={() => void mutate(() => apiFetch(`/projects/${projectId}/start`, { method: "POST" }))}>Start Work</button></section> : null}
      {project.editable && project.state === "ACTIVE" ? <><form className="portal-panel engineer-form action-anchor" id="updates" onSubmit={addNote}><p className="eyebrow">Field update</p><h2>Add Update</h2><textarea required minLength={3} value={note} onChange={(event) => setNote(event.target.value)} /><button className="engineer-secondary" disabled={busy} type="submit">Add Update</button></form><details className="portal-panel engineer-disclosure"><summary>Report Blocker</summary><form className="engineer-form" onSubmit={reportBlocker}><label>Blocker title<input required minLength={3} value={blockerTitle} onChange={(event) => setBlockerTitle(event.target.value)} /></label><label>Severity<select value={blockerSeverity} onChange={(event) => setBlockerSeverity(event.target.value)}><option>LOW</option><option>MEDIUM</option><option>HIGH</option><option>CRITICAL</option></select></label><label>Field details<textarea required minLength={10} value={blockerDetails} onChange={(event) => setBlockerDetails(event.target.value)} /></label><button className="engineer-secondary" disabled={busy} type="submit">Notify Project Head</button></form></details><section className="portal-panel action-card action-anchor" id="completion"><p className="eyebrow">Status update</p><h2>Work finished?</h2><button className="engineer-primary" disabled={busy} type="button" onClick={() => void mutate(() => apiFetch(`/projects/${projectId}/status`, { method: "PATCH", body: JSON.stringify({ state: "COMPLETED" }) }))}>Mark Complete</button></section></> : null}
      {project.editable && project.state === "COMPLETED" ? <form className="portal-panel engineer-form action-anchor" id="completion" onSubmit={submitCompletion}><p className="eyebrow">Completion evidence</p><h2>Submit Evidence</h2><label>Completion photo<input accept="image/jpeg,image/png,image/webp,image/heic" required type="file" onChange={(event) => setCompletionFile(event.target.files?.[0])} /></label><label>Notes (optional)<textarea value={completionNotes} onChange={(event) => setCompletionNotes(event.target.value)} /></label><button className="engineer-primary" disabled={busy || !completionFile} type="submit">Submit Evidence</button></form> : null}
      {project.state === "AWAITING_VERIFICATION" ? <section className="portal-panel verification-wait action-anchor" id="verification"><p className="eyebrow">Citizen verification</p><h2>Awaiting Verification</h2><p>Completion evidence has been sent to eligible citizen reviewers. This view will update automatically when quorum is reached.</p>{project.completionEvidence.map((evidence) => <a href={evidence.photoUrl} key={evidence.id} rel="noreferrer" target="_blank">View submitted evidence →</a>)}</section> : null}
