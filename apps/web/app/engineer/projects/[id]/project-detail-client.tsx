...
export function EngineerProjectDetailClient({ projectId }: { projectId: string }) {
  ...
  return <>
    ...
    {project.grievance ? <section className="portal-panel grievance-card-escalated"><p className="eyebrow">Linked grievance · {project.grievance.status.replaceAll("_", " ")}</p><h2>{project.grievance.reason.replaceAll("_", " ")}</h2><p>The responsible Project Head or Admin is reviewing this tracked escalation.</p></section> : null}
