...
import type { EngineerProjectDetail } from "@civicos/shared";
...
export function EngineerProjectDetailClient({ projectId }: { projectId: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setStart(dateInput(result.project.plannedStart, 0));
      setEnd(dateInput(result.project.plannedEnd, 7));
      setDescription(result.project.workDescription ?? "");
      setFlags(result.project.dependencyFlags.join(", "));
      setError(undefined);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not load project"); }
  }, [projectId]);
  useEffect(() => { void load(); }, [load]);
  const mutate = async (work: () => Promise<unknown>) => {
    setBusy(true); setError(undefined);
    try { await work(); await load(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Could not update project"); }
  ...
  const addNote = (event: FormEvent) => {
    event.preventDefault();
    void mutate(async () => { await apiFetch(`/projects/${projectId}/status`, { method: "PATCH", body: JSON.stringify({ note }) }); setNote(""); });
  };
  const submitCompletion = (event: FormEvent) => {
    event.preventDefault();
    if (!completionFile) return;
    void mutate(async () => {
      const target = await apiFetch<{ evidenceId: string; upload: { uploadUrl: string; headers: Record<string, string> } }>(`/projects/${projectId}/completion`, { method: "POST", body: JSON.stringify({ action: "presign", fileName: completionFile.name, contentType: completionContentType(completionFile), notes: completionNotes }) });
      ...
      setCompletionFile(undefined); setCompletionNotes("");
    });
  };
