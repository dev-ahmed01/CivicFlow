...
export function EngineerProjectList() {
  ...
  return <div className="field-module engineer-my-work">
    ...
    {view === "attention" && !loading ? <div className="engineer-attention-collection">{attentionInspections.map((item) => <Link key={item.id} href={`/engineer/inspections/${item.id}`}><span>Inspection &middot; {item.ticket.title}</span><span>{item.status === "ASSIGNED" ? "Review assignment" : "Continue inspection"} &rarr;</span></Link>)}{attentionDependencies.map((item) => <Link key={item.id} href={`/engineer/dependencies#dependency-${item.id}`}><span>Dependency &middot; {item.requirement}</span><span>View dependency &rarr;</span></Link>)}</div> : null}
