...
import { EngineerTip, EngineerSymbol, EngineerLoading } from "../_components/engineer-ui";
...
export default function EngineerDependenciesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(result.dependencies);
      setError(undefined);
    } catch (caught) {
      if (version === requestVersion.current) setError(caught instanceof Error ? caught.message : "Could not load dependencies");
    }
    finally { if (version === requestVersion.current) setLoading(false); }
  }, [direction]);
  usePortalPolling(load);
  useEffect(() => {
    if (loading || followedHash.current || !window.location.hash.startsWith("#dependency-")) return;
    const record = document.getElementById(window.location.hash.slice(1));
    if (record) { record.scrollIntoView({ block: "center" }); record.focus({ preventScroll: true }); followedHash.current = true; }
  }, [loading, items]);
  ...
  return <div className="engineer-dependencies-page">
    <header className="portal-heading"><div><p className="eyebrow">Agency coordination</p><h1>Dependencies</h1><p>See how your project connects to partner agencies, then act on the current dependency state.</p></div><div className="engineer-dependency-summary"><strong>{loading ? "—" : direction === "received" ? responseCount + assignedToMe : items.length}</strong><span>{direction === "received" ? "actionable tasks" : "requests sent"}</span></div></header>
    <div aria-label="Dependency views" className="engineer-work-tabs" role="tablist"><button aria-controls="engineer-dependency-results" aria-selected={direction === "received"} className={direction === "received" ? "active" : ""} onClick={() => { if (direction === "received") return; setLoading(true); setItems([]); setDirection("received"); setExpandedId(undefined); }} role="tab" type="button">Received</button><button aria-controls="engineer-dependency-results" aria-selected={direction === "sent"} className={direction === "sent" ? "active" : ""} onClick={() => { if (direction === "sent") return; setLoading(true); setItems([]); setDirection("sent"); setExpandedId(undefined); }} role="tab" type="button">Sent</button></div>
    ...
    <div className="engineer-stat-grid">{[
      ...
      { value: items.filter((item) => item.state === "ESCALATED").length, label: "Blocked", note: items.some((item) => item.state === "ESCALATED") ? "Escalated requests" : "No blockers", tone: "red", icon: "blocked" },
    ].map((stat) => <article className="engineer-stat" key={stat.label}><span className={"engineer-symbol " + stat.tone}><EngineerSymbol name={stat.icon} /></span><div><strong>{loading ? "—" : stat.value}</strong><span>{stat.label}</span><small>{stat.note}</small></div></article>)}</div>
    {overdue[0] ? <section className="engineer-attention"><p className="eyebrow">Needs your attention</p><div><span className="engineer-symbol amber"><EngineerSymbol name="bell" /></span><div><strong>{overdue.length} {overdue.length === 1 ? "dependency is" : "dependencies are"} overdue</strong><p>{overdue[0].project.ticket?.title ?? "Agency coordination"}</p><span>From {overdue[0].requestingAgency.name} &middot; <b>Overdue by {Math.ceil((Date.now() - new Date(overdue[0].deadline).getTime()) / 86400000)} days</b></span></div><button className="next-action-button primary" type="button" onClick={() => { setExpandedId(overdue[0]!.id); const target = document.getElementById("dependency-" + overdue[0]!.id); target?.scrollIntoView({ behavior: "smooth", block: "center" }); target?.focus({ preventScroll: true }); }}>Review now &rarr;</button></div></section> : null}
    ...
    <section aria-live="polite" className="dependency-flow-grid" id="engineer-dependency-results" role="tabpanel" tabIndex={0} aria-busy={loading}>{loading ? <EngineerLoading label="Loading dependencies" /> : null}{sorted.map((item) => {
      ...
      return <div id={"dependency-" + item.id} key={item.id} tabIndex={-1}><EngineerDependencyCard dependency={item} direction={direction} projectHref={`/engineer/projects/${item.project.id}`}>
    ...
    })}{!loading && items.length === 0 ? <div className="empty-state engineer-dependency-empty"><strong>No {direction} dependency requests.</strong><span>Coordination requests will appear automatically when projects connect agency work.</span></div> : null}</section>
