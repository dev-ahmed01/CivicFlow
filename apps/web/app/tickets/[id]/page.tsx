...
export default function CitizenTicketDetailPage() {
  ...
  useEffect(() => { const accessToken = getCitizenAccessToken(); void fetch(`${apiUrl}/tickets/${params.id}`, { headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : {}, cache: "no-store" }).then(async (response) => { const body = await response.json() as { ticket?: CitizenTicketSummary; error?: string }; if (!response.ok || !body.ticket) throw new Error(body.error ?? "Could not load ticket"); setTicket(body.ticket); }).catch((caught: unknown) => setError(caught instanceof Error ? caught.message : "Could not load ticket")); }, [params.id]);
