...
export function ReportForm() {
  ...
  if (result) return <section className="confirmation" aria-live="polite"><span className="success-mark">✓</span><p className="eyebrow">Report submitted</p><h2>{result.title}</h2><p className="ticket-id">Ticket ID: {result.id}</p><p className="status-pill">{result.statusLabel}</p><button type="button" onClick={() => setResult(undefined)}>Done</button></section>;
  ...
  return <form className="report-form" onSubmit={(event) => void submit(event)}>
    <section className="form-section"><div className="step">1</div><div className="section-content"><p className="eyebrow">Issue category</p><h2>What needs attention?</h2><input aria-label="Search categories" placeholder="Search issue types" value={query} onChange={(event) => setQuery(event.target.value)} /><div className="category-grid">{filtered.map((category) => <button className={categoryId === category.id ? "category active" : "category"} type="button" key={category.id} onClick={() => setCategoryId(category.id)}><span>{category.name.slice(0, 1)}</span>{category.name}</button>)}</div></div></section>
