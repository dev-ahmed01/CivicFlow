...
export function RoadIntelligencePanel({ data, projectId, plannedStart, plannedEnd, onAction }: Props) {
  ...
  return <div className="road-intelligence-stack">
    {data.conflicts.length ? <ConflictBanner><p className="eyebrow">Road-specific advisory check</p><h2>{data.conflicts.length} road-cutting warning{data.conflicts.length === 1 ? "" : "s"}</h2><p>Exact-segment checks run after the generic engine. Continue anyway remains available.</p><div className="conflict-list">{data.conflicts.map((conflict) => <article className={`conflict-item road-${conflict.severity.toLowerCase().replace("_", "-")}`} key={conflict.id}><div><strong>{title(conflict.type)}</strong><StatusChip label={conflict.severity} tone="warning" /></div><p>{conflict.segmentName}{conflict.conflictingAgency ? ` · ${conflict.conflictingAgency.name}` : " · Single-record risk"}</p><small>{conflict.reason}</small></article>)}</div></ConflictBanner> : null}
    {data.recommendations.map((recommendation) => <SequencingRecommendationCard key={recommendation.id}><div className="sequencing-heading"><div><p className="eyebrow">Sequencing recommendation · Advisory</p><h2>Explainable work order</h2></div><StatusChip label={recommendation.latestOutcome ?? "Pending review"} tone={recommendation.latestOutcome ? "info" : "warning"} /></div><p className="sequencing-explanation">{recommendation.explanation}</p><ol className="sequencing-order">{recommendation.proposedOrder.map((item, index) => <li key={`${item.interventionId ?? "restoration"}-${index}`}><span>{index + 1}</span><div><strong>{item.purpose}</strong><small>{item.agencyName} · {new Date(item.plannedStart).toLocaleDateString("en-IN")} – {new Date(item.plannedEnd).toLocaleDateString("en-IN")}</small></div></li>)}</ol><details><summary>Why this order?</summary><ol className="rule-trace">{recommendation.ruleTrace.map((trace) => <li key={trace.rule}><strong>Rule {trace.rule}</strong> {trace.reason}</li>)}</ol></details>{onAction ? <div className="recommendation-actions"><div className="revision-dates"><label>Revised start<input type="date" value={start} onChange={(event) => setStart(event.target.value)} /></label><label>Revised end<input type="date" min={start} value={end} onChange={(event) => setEnd(event.target.value)} /></label></div><div><button disabled={Boolean(busy)} type="button" onClick={() => void act(recommendation.id, "ACCEPTED")}>Accept{start && end ? " with dates" : ""}</button><button className="secondary" disabled={Boolean(busy)} type="button" onClick={() => void act(recommendation.id, "MODIFIED")}>Modify</button><button className="dismiss" disabled={Boolean(busy)} type="button" onClick={() => void act(recommendation.id, "DISMISSED")}>Dismiss</button></div><small>Every action is logged. Dismissal never deletes the recommendation.</small></div> : null}</SequencingRecommendationCard>)}
