...
export function VerificationPanel() {
  ...
  const updateLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition((position) => { void apiFetch("/citizens/me/location", { method: "PATCH", body: JSON.stringify({ latitude: position.coords.latitude, longitude: position.coords.longitude }) }).then(() => apiFetch<{ validations: PendingValidation[] }>("/citizens/me/pending-validations")).then((result) => { setValidations(result.validations); setMessage("Your current location has been updated."); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Could not update your location")); }, () => setError("Allow location access to find nearby requests"), { enableHighAccuracy: true, timeout: 15000 });
  };
