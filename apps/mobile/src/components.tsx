...
export function CategoryGrid({ categories, selectedId, onSelect }: { categories: CategorySummary[]; selectedId?: string; onSelect: (category: CategorySummary) => void }) {
  ...
  return <View accessibilityLabel="Issue categories" style={styles.categoryGrid}>{categories.map((category) => {
    ...
    return <Pressable accessibilityRole="button" accessibilityState={{ selected }} key={category.id} onPress={() => onSelect(category)} style={({ pressed }) => [styles.categoryTile, { backgroundColor: selected ? palette.accent : palette.surface, borderColor: selected ? palette.accentStrong : palette.border }, pressed && styles.pressed]}>
...
export function MobileTabBar({ active, items, onSelect }: { active: string; items: MobileTab[]; onSelect: (id: string) => void }) {
  ...
  return <View accessibilityRole="tablist" style={[styles.tabBar, { backgroundColor: palette.surface, borderTopColor: palette.border, paddingBottom: Math.max(insets.bottom, 6) }]}>{items.map((item) => {
    ...
    return <Pressable accessibilityRole="tab" accessibilityState={{ selected }} key={item.id} onPress={() => onSelect(item.id)} style={styles.tab}>
      ...
      <Text numberOfLines={1} style={[styles.tabLabel, { color: selected ? palette.accentStrong : palette.textSecondary }]}>{item.label}</Text>
    </Pressable>;
  })}</View>;
}
...
const styles = StyleSheet.create({
  ...
  header: { gap: 6, marginBottom: 8 }, headerTop: { alignItems: "center", flexDirection: "row", justifyContent: "space-between", minHeight: 34 }, backButton: { alignItems: "center", flexDirection: "row", marginBottom: 6, marginLeft: -5, minHeight: 36 }, back: { fontSize: typeScale.body, fontWeight: fontWeights.semibold },
  ...
  primaryButton: { alignItems: "center", borderRadius: radii.button, flexDirection: "row", gap: 9, justifyContent: "center", minHeight: 52, paddingHorizontal: 20, paddingVertical: 14 }, primaryLabel: { fontSize: typeScale.input, fontWeight: fontWeights.semibold }, secondaryButton: { alignItems: "center", borderRadius: radii.button, borderWidth: 1, flexDirection: "row", gap: 9, justifyContent: "center", minHeight: 50, paddingHorizontal: 18, paddingVertical: 13 }, secondaryLabel: { fontSize: typeScale.body, fontWeight: fontWeights.semibold }, pressed: { opacity: 0.78 }, disabled: { opacity: 0.45 },
  card: { borderRadius: radii.card, borderWidth: 1, elevation: 1, gap: 10, padding: 17, shadowColor: "#0A2415", shadowOffset: { height: 3, width: 0 }, shadowOpacity: 0.05, shadowRadius: 10 }, chip: { alignSelf: "flex-start", borderRadius: radii.chip, fontSize: typeScale.caption, fontWeight: fontWeights.semibold, overflow: "hidden", paddingHorizontal: 10, paddingVertical: 5 },
  ticketTop: { alignItems: "flex-start", flexDirection: "row", gap: 10, justifyContent: "space-between" }, ticketId: { fontSize: typeScale.caption, fontWeight: fontWeights.semibold }, ticketTitle: { fontSize: typeScale.input, fontWeight: fontWeights.semibold, lineHeight: 22 }, ticketMeta: { fontSize: typeScale.body, lineHeight: 20 }, metaRow: { alignItems: "flex-start", flexDirection: "row", gap: 6 }, metaCopy: { flex: 1 }, ticketPressTarget: { gap: 10 },
  ...
  categoryGrid: { flexDirection: "row", flexWrap: "wrap", gap: 11 }, categoryTile: { alignItems: "flex-start", borderRadius: 18, borderWidth: 1, gap: 9, minHeight: 128, padding: 15, width: "48%" }, categoryIcon: { alignItems: "center", borderRadius: 20, height: 40, justifyContent: "center", width: 40 }, categoryLabel: { flex: 1, fontSize: typeScale.body, fontWeight: fontWeights.semibold, lineHeight: 19 },
  tabBar: { borderTopWidth: 1, flexDirection: "row", minHeight: 66, paddingHorizontal: 4, paddingTop: 7 }, tab: { alignItems: "center", flex: 1, gap: 4, minHeight: 52, paddingHorizontal: 2, paddingVertical: 3 }, tabLabel: { fontSize: 10, fontWeight: fontWeights.semibold }, tabBadge: { alignItems: "center", backgroundColor: citizenColors.dangerText, borderColor: citizenColors.surface, borderRadius: 8, borderWidth: 1.5, height: 17, justifyContent: "center", minWidth: 17, paddingHorizontal: 3, position: "absolute", right: -9, top: -5 }, tabBadgeText: { color: citizenColors.surface, fontSize: 9, fontWeight: "700" },
});
