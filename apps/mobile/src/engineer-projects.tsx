...
import { ActivityIndicator, Alert, AppState, FlatList, Image, Modal, Pressable, ScrollView, StyleSheet, Text, TextInput, View } from "react-native";
import {
  internalLogin,
  loadAgencies,
  loadWorkflowOptions,
...
function TimelineScreen({ project, onBack, onSaved }: { project: EngineerProjectDetail; onBack: () => void; onSaved: () => void }) {
  ...
  useEffect(() => { void loadWorkflowOptions().then((result) => setDemo(result.demoDefaults)).catch(() => setDemo(false)); }, []);
...
function CompletionScreen({ projectId, onBack, onSubmitted }: { projectId: string; onBack: () => void; onSubmitted: () => void }) {
  ...
  const pick = async () => {
    ...
    if (!permission.granted) { Alert.alert("Camera permission needed", "Enable camera access in Android settings to add completion evidence."); return; }
    const result = await ImagePicker.launchCameraAsync({ mediaTypes: ["images"], quality: 0.85 });
    if (!result.canceled) {
      const asset = result.assets[0];
      if (!asset) return;
      const extension = asset.uri.split(".").pop()?.toLowerCase();
      const contentType: LocalImage["contentType"] = asset.mimeType === "image/png" || extension === "png" ? "image/png" : asset.mimeType === "image/webp" || extension === "webp" ? "image/webp" : asset.mimeType === "image/heic" || asset.mimeType === "image/heif" || extension === "heic" || extension === "heif" ? "image/heic" : "image/jpeg";
  ...
  return <Shell><ScrollView contentContainerStyle={styles.content}><ScreenHeader eyebrow="Completion evidence" title="Submit for verification" onBack={onBack} /><Pressable accessibilityRole="button" onPress={() => void pick()} style={styles.photo}>{image ? <Image source={{ uri: image.uri }} style={styles.photoImage} /> : <Text style={styles.photoPrompt}>Take completion photo</Text>}</Pressable><TextInput accessibilityLabel="Completion notes (optional)" multiline value={notes} onChangeText={setNotes} placeholder="Completion notes (optional)" placeholderTextColor={colors.muted} style={[styles.input, styles.textarea]} /><PrimaryButton disabled={busy || !image} onPress={() => void submit()}>{busy ? "Submitting…" : "Submit for Verification"}</PrimaryButton></ScrollView></Shell>;
}
...
function ProjectDetailScreen({ projectId, onBack, onTimeline, onCompletion }: { projectId: string; onBack: () => void; onTimeline: (project: EngineerProjectDetail) => void; onCompletion: () => void }) {
  ...
  const start = async () => { try { await startProject(projectId); await refresh(); } catch (caught) { Alert.alert("Couldn't start work", errorMessage(caught)); } };
  ...
  return <Shell><ScrollView contentContainerStyle={styles.content}>
    <ScreenHeader eyebrow={project.editable ? "Assigned task" : "Area project · Read-only"} title={project.ticket?.title ?? project.title} onBack={onBack} />
    <Text style={styles.chip}>{stateLabel(project.state)}</Text>
    <View style={styles.taskBrief}><Text style={styles.kicker}>Location</Text><Text style={styles.cardTitle}>{project.ticket?.address ?? project.locationLabel ?? "Location not recorded"}</Text><Text style={styles.meta}>{project.ticket?.ward.name ?? "Ward unavailable"} · {project.agency.name}</Text></View>
    ...
    <View style={styles.taskBrief}><Text style={styles.kicker}>Inspection</Text>{project.ticket?.inspectionReports.length ? project.ticket.inspectionReports.map((report) => <View key={report.id}><Text style={styles.bodyText}>{report.observations ?? report.notes ?? "Inspection record available"}</Text><Text style={styles.meta}>{stateLabel(report.status)} · {report.recommendation ? stateLabel(report.recommendation) : "Recommendation pending"}</Text></View>) : <Text style={styles.meta}>No inspection is attached. Confirm requirements with the Project Head before execution.</Text>}</View>
    ...
    {project.editable && project.state === "PENDING_UPTAKE" ? <PrimaryButton onPress={() => void accept()}>Accept Assignment</PrimaryButton> : null}
    {project.editable && ["UPTAKEN", "TIMELINE_SET", "CONFLICT_CHECKED", "READY_TO_START", "ACTIVE", "MODIFIED"].includes(project.state) ? <SecondaryButton onPress={() => onTimeline(project)}>{project.plannedStart ? "Update Timeline" : "Save Timeline"}</SecondaryButton> : null}
    {project.editable && project.state === "READY_TO_START" ? <><Text style={styles.readOnly}>Planning and advisory checks are complete. Starting work records the actual start time.</Text><PrimaryButton onPress={() => void start()}>Start Work</PrimaryButton></> : null}
    {project.editable && project.state === "ACTIVE" ? <><TextInput accessibilityLabel="Field update" multiline value={note} onChangeText={setNote} placeholder="Add a field update" placeholderTextColor={colors.muted} style={[styles.input, styles.textarea]} /><SecondaryButton onPress={() => void update(false)}>Save field update</SecondaryButton><PrimaryButton onPress={() => void update(true)}>Mark Work Completed</PrimaryButton></> : null}
    {project.editable && project.state === "COMPLETED" ? <PrimaryButton onPress={onCompletion}>Upload evidence & report</PrimaryButton> : null}
    {!project.editable ? <Text style={styles.readOnly}>Only the assigned engineer can update this work.</Text> : null}
  </ScrollView></Shell>;
}
...
export function EngineerProjectsApp({ auth, onLogout }: { auth: CurrentAuth; onLogout: () => void }) {
  ...
  useEffect(() => {
    const refreshUnread = () => {
      void loadNotifications(true).then((result) => setNotificationUnread(result.unreadCount)).catch(() => undefined);
    };
    const unsubscribePush = subscribeToPushReceipt(refreshUnread);
  ...
  if (screen === "notifications") return withNavigation(<NotificationsScreen role="ENGINEER" onBack={() => setScreen("mine")} onOpen={openNotification} onViewed={setNotificationUnread} />);
  ...
  if (screen === "completion" && projectId) return withNavigation(<CompletionScreen projectId={projectId} onBack={() => setScreen("detail")} onSubmitted={() => { Alert.alert("Submitted", "Eligible citizen reviewers have been notified."); setScreen("detail"); }} />);
  return withNavigation(<ProjectsScreen scope="mine" auth={auth} onOpen={openProject} />);
}

const styles = StyleSheet.create({
  app: { backgroundColor: colors.canvas, flex: 1 }, stage: { flex: 1, minWidth: 0 }, tabInset: { backgroundColor: colors.canvas },
  ...
  dashboardCard: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 12, borderWidth: 1, flexGrow: 1, gap: 7, minHeight: 140, minWidth: 145, padding: 17, width: "47%" },
  ...
  photo: { alignItems: "center", aspectRatio: 4 / 3, backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 20, borderStyle: "dashed", borderWidth: 2, justifyContent: "center", maxHeight: 280, overflow: "hidden", width: "100%" },
