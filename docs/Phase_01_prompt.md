...
- `User` (id, role: CITIZEN | PROJECT_HEAD | ENGINEER | ADMIN, phone (citizens) or email (internal), auth fields, agencyId nullable, wardId nullable, createdAt)
...
- `Category` (id, name, primaryAgencyId, admin-editable — seed with the 12 categories: Road Damage, Streetlight, Water Supply, Drainage/Sewage, Garbage/Waste, Electrical Hazard, Public Toilet, Parks & Trees, Stray Animals, Illegal Construction, Traffic & Signage, Other)
...
- `AdminConfig` (key, value, description) — for tunable thresholds (verification radius, duplicate detection thresholds, validation daily cap, etc. — do not hardcode these anywhere in app logic, read from this table)
...
- Project Head / Engineer / Admin: email + password, bcrypt hashed, first-login forces password reset. Admin gets optional TOTP 2FA field on the schema (implementation can follow later, just don't block on it now).
