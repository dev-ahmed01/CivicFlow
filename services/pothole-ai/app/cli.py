...
def draw_annotations(img_bgr: np.ndarray, detections: List[PotholeDetection]) -> np.ndarray:
    ...
    for det in detections:
        ...
        label_text = f"Pothole {det.confidence:.2f} ({det.visualSeverityCandidate})"
        cv2.putText(
            annotated,
            label_text,
            (bx, max(20, by - 8)),
...
def process_video_file(
    ...
    and optionally renders an annotated demo video and JSON detection report.
    """
    if not HAS_CV2:
        print("[ERROR] Processing video files requires OpenCV (cv2). Please install 'opencv-python-headless'.")
        sys.exit(1)

    print(f"[CLI] Opening video file: {video_path}")
    ...
    while cap.isOpened():
        ...
        if current_frame_idx % frame_step == 0:
            ...
            print(f"[FRAME {current_frame_idx:05d} | {timestamp_sec:05.1f}s] Detections: {len(detections)}")
    ...
    print("\n=================== DEMO SCAN REPORT ===================")
    print(f"Sampled Frames Processed : {len(frame_inputs)}")
    ...
    for cluster in temporal_summary.confirmedClusters:
        print(f"  - Defect {cluster.clusterId}: Observed in {cluster.repeatCount} frames (First: {cluster.firstTimestampSec}s, Last: {cluster.lastTimestampSec}s, AvgConf: {cluster.averageConfidence:.2f}, Severity: {cluster.visualSeverityCandidate})")
    print("========================================================\n")
